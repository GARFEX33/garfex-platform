...
These are four distinct responsibilities: TypeSpec owns client-safe meaning; the trusted edge owns authentication, fresh actor construction, explicit named mappings, projection, and safe normalization; Resource Master owns the final deny-by-default capability check; native Convex is a downstream adapter for GARFEX-owned local/development clients only. Revision `1` is opaque and compared exactly; HTTP, productive identity, provider, UI, SDK, publication, and version-policy decisions remain unselected.
