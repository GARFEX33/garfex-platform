...
describe("Convex persistent Resource Catalog reader", () => {
  it("classifies zero, one, and duplicate documents through the indexed bounded path", async () => {
    ...
    await expect(t.run((ctx) => new ConvexResourceCatalogReader(ctx.db).loadSnapshot())).rejects.toMatchObject({
      code: "RESOURCE_CATALOG_UNINITIALIZED",
    });
    await t.run((ctx) => ctx.db.insert("resourceCatalogSnapshots", snapshot as never));
    expect(await t.run((ctx) => new ConvexResourceCatalogReader(ctx.db).loadSnapshot())).toEqual(snapshot);
    await t.run((ctx) => ctx.db.insert("resourceCatalogSnapshots", snapshot as never));
    await expect(t.run((ctx) => new ConvexResourceCatalogReader(ctx.db).loadSnapshot())).rejects.toMatchObject({
  ...
  it("maps empty and storage failures without leaking implementation detail", async () => {
    ...
    const empty = {
      ...snapshot,
      catalog: { ...snapshot.catalog, attributes: [], optionSets: [], naturalUnits: [], bindings: [], presentation: { attributeOrder: [], includeNaturalUnit: false } },
    };
    await t.run((ctx) => ctx.db.insert("resourceCatalogSnapshots", empty as never));
    await expect(t.run((ctx) => new ConvexResourceCatalogReader(ctx.db).loadSnapshot())).rejects.toMatchObject({
      code: "RESOURCE_CATALOG_UNINITIALIZED",
    });
    const db = { query: () => { throw new Error("storage"); } } as never;
  ...
  it("measures the payload under conservative bounds and uses take(2)", async () => {
    expect(measureResourceCatalog(cableCatalogV1)).toEqual({ bytes: 3317, depth: 8, largestArray: 5, largestObject: 9 });
    expect(measureResourceCatalog(cableCatalogV1).bytes).toBeLessThanOrEqual(resourceCatalogBounds.maxBytes);
    const calls: number[] = [];
    const db = { query: () => ({ withIndex: () => ({ take: (limit: number) => { calls.push(limit); return Promise.resolve([]); } }) }) } as never;
    await expect(new ConvexResourceCatalogReader(db).loadSnapshot()).rejects.toMatchObject({
      code: "RESOURCE_CATALOG_UNINITIALIZED",
    });
